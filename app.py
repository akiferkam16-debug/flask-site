...
def index():
    ...
    <head>
        ...
        <style>
            ...
            .products-section h2 {{ text-align:center; color:#ffd700; }}
            .products-grid {{ display:grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap:20px; justify-items:center; }}
            .product-card {{ background:#222; border-radius:12px; padding:12px; width:200px; text-align:center; color:#fff; transition: 0.3s; }}
    ...
    <body>
        <header>
            <a class="cart-link" href="{{{{ url_for('cart_page') }}}}" id="cart-btn">🛒 Sepet</a>
        ...
        <footer>&copy; 2025 Erkam Mıknatıs.</footer>
    </body>
    </html>
    """)

# --- SEPET MANTIĞI: EKLE / ARTIR ---
@app.route("/add_to_cart/<int:product_id>")
def add_to_cart(product_id):
    # Sepet bilgisini sözlük olarak alıyoruz {}
    cart = session.get("cart", {})
    str_id = str(product_id) # JSON anahtarları için string yapıyoruz
    
    if str_id in cart:
        # Ürün zaten varsa miktarını artır
        cart[str_id]['quantity'] += 1
    else:
        # Ürün yoksa tüm listelerde ara ve ekle
...
def cart_page():
    ...
    for item_id, item in cart.items():
        # Fiyatı sayıya çevir (3.00 TL -> 3.0)
        try:
            clean_price = float(item["price"].replace(" TL", "").replace(",", "."))
        ...
        <div class="cart-item">
            <div class="item-info">
                ...
                <span>Birim: {item['price']}</span>
            </div>
            <div class="qty-box">
                <a class="qty-btn" href="{url_for('remove_from_cart', product_id=item['id'])}">-</a>
                <span class="qty-num">{item['quantity']}</span>
                <a class="qty-btn" href="{url_for('add_to_cart', product_id=item['id'])}">+</a>
            </div>
            <div class="item-total">{subtotal:.2f} TL</div>
        </div>
        """
    ...
    <head>
        ...
        <style>
            ...
            .cart-container {{ max-width:600px; margin: 0 auto; background:#1e2a4a; padding:20px; border-radius:10px; }}
            h1 {{ color:#ffd700; text-align:center; }}
            .cart-item {{ display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid #334; padding:15px 0; }}
            .qty-box {{ display:flex; align-items:center; gap:10px; }}
            .qty-btn {{ background:#ffd700; color:#000; text-decoration:none; padding:5px 10px; border-radius:4px; font-weight:bold; }}
            .qty-num {{ font-size:1.2em; min-width:20px; text-align:center; }}
            .total-area {{ text-align:right; margin-top:20px; font-size:1.5em; color:#ffd700; border-top:2px solid #ffd700; padding-top:10px; }}
            .back-btn {{ display:inline-block; margin-top:20px; color:#fff; text-decoration:none; border:1px solid #fff; padding:10px; border-radius:5px; }}
            .confirm-btn {{ display:block; background:green; color:#fff; text-align:center; padding:15px; border-radius:8px; text-decoration:none; margin-top:20px; font-weight:bold; }}
        </style>
    </head>
    <body>
        <div class="cart-container">
            <h1>🛒 Sepetiniz</h1>
            {cart_items_html if cart_items_html else '<p style="text-align:center;">Sepetiniz şu an boş.</p>'}
            <div class="total-area">Toplam: {grand_total:.2f} TL</div>
            <a href="/" class="back-btn">⬅️ Alışverişe Dön</a>
            <a href="#" class="confirm-btn">💳 Sepeti Onayla</a>
        </div>
    </body>
    </html>
    """)
